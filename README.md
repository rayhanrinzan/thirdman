...
The same frames drive playback and final application. Validation checks both the displayed defensive trajectories and every opponent's ability to run directly into the passing lane: it never assumes a defender will just follow the ball slowly. A continuous reach envelope uses a 180 ms reaction delay, six pitch-length units per second of closing speed, and the existing four-unit clearance. This also covers defenders outside the two active pressers, goalkeepers, and a 200 ms first-touch window at the receiver. These are conservative simulation settings, not measured player attributes. Each pass must also clear the initial lane and a swept ball-versus-defender check between simulation ticks. Pass duration is normalized from distance, with a one-second minimum and a 2.4-second cap; changing model-provided timing cannot bypass the check. Long unsupported direct passes require intermediate teammates. The travelling ball is centered on its simulated coordinates; only a ball held by a player uses a decorative offset.
